            ...
            <div class="card-header">
                @can('parameter:add')
                    <div class="card-toolbar">
                        <!--begin::Button-->
                        <a href="{{ url(adminUrl('visit-places/create')) }}" class="btn btn-primary font-weight-bolder">
...
<script>
    ...
    DataTableHelper.initDatatable('#datatable', [0, 'desc'], 'POST', '{{ adminUrl('visit-places/list') }}', [
            {
                title: 'ID',
                data: 'id',
            ...
            },
            {
                ...
                render: function(data, type, row)
                {
                    let  html = '<a href="{{ adminUrl('visit-places') }}/'+ data +'/edit" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>';
                    return data ? html : "";
                }
            },
            {
                ...
                render: function(data, type, row)
                {
                    let  html = '<button onclick="DataTableHelper.deleteRecord(\'{{ adminUrl('visit-places') }}/'+ data +'\')" class="btn btn-sm btn-clean btn-icon" title="Delete"><i class="la la-remove"></i></button>';
                    return data ? html : "";
                }
            },
        ],
