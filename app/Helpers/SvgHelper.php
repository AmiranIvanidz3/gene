...
class SvgHelper{
    public static function generateDataSvgHtml($data, $svg_width = 100, $svg_height = 50, $price_alias = 'price', $settings = []){ //, $debug_id = false
    ...
    public static function job(){
        // Retrieves all SD tokens
        $items = DB::table('tokens')->get()->toArray();

        // Get token's SVGs
        $tokenSVGs = self::getTokenSvgs($items);
        // dd($tokenSVGs);
        // loop through tokens to generate SVGs
        foreach($items as $item) {
            ...
            //     return;
            // }
            // Updates price and [price change 24h] into "blockchains"
            DB::table('tokens')
                ->where('id', $item['id'])
                ->update([
                    'svg_usd_1w' => $tokenSVGs[$item['id']]['usd_1w']['json'] ?? '',
                    'svg_pair_1w' => $tokenSVGs[$item['id']]['pair_1w'] ?? '',
                    
                    'svg_usd_1m' => $tokenSVGs[$item['id']]['usd_1m'],
                    'svg_pair_1m' => $tokenSVGs[$item['id']]['pair_1m'],

                    // 'svg_usd_1m_homepage' => $tokenSVGs[$item['id']]['usd_1m']['homepage'],
                    'svg_homepage' => $tokenSVGs[$item['id']]['usd_1w']['homepage']  ?? '',
                ]
            );	
            // dd($item, $tokenSVGs);
        }
    ...
    public static function TokenAllTime($token_id, $price_alias = 'market_price_usd'){
        ...
        $svg = Cache::remember('token.all_time.'.$token_id.'.'.$price_alias, $cache_duration, function() use($token_id, $price_alias, $dot_limit){

            $cnt_prices = DB::table('sdt_prices_unique')->where('token_id', $token_id)->count();
            ...
            if($cnt_prices > $dot_limit){
                ...
                $data = DB::table('sdt_prices_unique')
					->where('token_id', $token_id)
					->orderBy('created_at')
					->get();
            }
    ...
    public static function tokenBaseData($token_id, $time = '1w', $type = 'pair'){
        ...
        $svg = Cache::remember('token.base.'.$token_id.'.'.$time.'-'.$type, $cache_duration, function() use($token_id, $price_alias, $dot_limit, $unit_count){
