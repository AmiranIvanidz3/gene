...
;(function ( $, window, document, undefined ) {
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    if(sourceItem.toFilter){
      filterDataHtml = 'data-to-filter="'+sourceItem.toFilter+'" ';
    }
    if (this.options.isMultiple)
      itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" '+filterDataHtml+'class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" />' : '') + sourceItem.title + '</span>';
    else
      itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" '+filterDataHtml+'class="comboTreeItemTitle ' + selectableClass + '">' + sourceItem.title + '</span>';
  ...
  ComboTree.prototype.bindings = function () {
    ...
    console.log('this.options.filterSelector', this.options.filterSelector);
    if(typeof this.options.filterSelector){
      $(this.options.filterSelector).on('change', function(){
        console.log('filterSelector On Change');
        _this.customFilter($(this).val())
      })
    }
  };
  ...
  ComboTree.prototype.dropDownInputKeyControl = function (step) {
    ...
    ComboTree.prototype.filterDropDownMenu = function () {
      var searchText =  '';
      let isFiltered = this.options.isFiltered;
      let _elemItems, _elemItemsTitle;

      if(isFiltered){
        _elemItems = this._elemItems.filter('.custom-filtered');
        _elemItemsTitle = _elemItems.find('span.comboTreeItemTitle');
      }else{
        _elemItems = this._elemItems;
        _elemItemsTitle = this._elemItemsTitle;
      }
      if (!this.options.isMultiple)
      ...
      if (searchText != ""){
        _elemItemsTitle.hide();
        _elemItemsTitle.siblings("span.comboTreeParentPlus").hide();
        list = _elemItems.filter(function(index, item){
        ...
        _elemItemsTitle.show();
        _elemItemsTitle.siblings("span.comboTreeParentPlus").show();

      }
    }
    ComboTree.prototype.customFilter = function (toFilter) {
      var searchText = toFilter;
      console.log('FILTER ACTIVATED', searchText);
      if (searchText != ""){
        this.options.isFiltered = true;
        this._elemItems.removeClass('custom-filtered');
        this._elemItemsTitle.hide();
        this._elemItemsTitle.siblings("span.comboTreeParentPlus").hide();
        list = this._elemItems.filter(function(index, item){
          // console.log('item.dataset.toFilter', item.dataset.toFilter);
          // console.log('item.dataset', item.dataset);
          // console.log('item', item);
          // console.log('–––––––––');
          let data_to_filter = item.getElementsByClassName('comboTreeItemTitle')[0].dataset.toFilter;
          if(!data_to_filter){
            return false;
          }

          return data_to_filter.indexOf(searchText) != -1;
        }).each(function (i, elem) {
          $(this).addClass('custom-filtered');
          $(this.children).show()
          $(this).siblings("span.comboTreeParentPlus").show();
        });
      }
