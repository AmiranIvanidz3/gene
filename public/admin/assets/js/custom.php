...
function checkFB(e){
    $('.query_string').val(e.target.checked ? 'fbclid='  : "").trigger('change')
}
checkMobile()
...
function createModal(data, model){
    ...
    return `
                <button type="button" class="btn btn-primary btn-open-modal" data-toggle="modal" data-target="#comment_${data}">
                    Comment
                </button>
...
function createCommentTable(model, model_id, isAdmin){
    ...
    render: function(data, type, row){
       ...
       DataTableHelper.initDatatable('#datatable_comment', [0, 'desc'], 'POST', `/sdtadmin/comment/list/${model}/${model_id}`,
       columns,
       false,
       [
        {
...
let User = {
    url : '/sdtadmin/api/roles/role_id/permissions',
    config : {
    ...
    checkPermissionsByRole : function (){
        ...
        $.get(APIurl, {}, function (data){
            ...
            rolePermissions.forEach(function myFunction(item, index) {
...
$(document).on('mouseover', '.has-tooltip', function(e) {
  ...
  function copyReel(title, id) {
    let link = title.replace(/ /g, "-").replace(/❓/g, "?").replace(/,/g, "").replace(/\./g, "");
    link = `https://wisdom.ge/${id}/${link}`
